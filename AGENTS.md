...
- A redirect out of a proxied POST must be an **absolute** storefront URL (`portalRedirectTarget`). Shopify follows 30x responses itself and resolves a relative `Location` against the app's origin, so `/apps/plant-requests` becomes a path the app does not serve and the customer gets "Bad Request" after the request was already saved.
