...
- Statuses stored: New / Pending / Closed / Expired. Customer labels are derived by `formatCustomerStatusLabel`: Pending and unanswered is **Offer Ready for Review**, answered with something payable is **Needs Payment**, and nothing payable is **No Payment Needed**. Labels change, stored statuses do not. Rejecting every purchasable plant closes the request immediately (`Customer Closed Request`); do not leave those requests Pending while the declined plants go to EXACT PLANTS.
...
- **Owner decision:** a declined exact plant stays eligible for the EXACT PLANTS queue even once the request is `Closed`, and even after a sibling on the same request is paid. Payment of **that plant** decides eligibility, not the request status and not request-level `paidAt`. `Closed` means paid *or* closed because there was nothing to pay for; only an accepted-and-paid (or `purchasedAt`) plant is out of reach. Do not reintroduce a bare `Closed` check or a blanket `paidAt` check in `exactPlantReleaseReason`.
