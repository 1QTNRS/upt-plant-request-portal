...
- Offer send with 3/5/7 day hold; offer snapshot freezes name, price, weight, photos, notes, availability, fulfilment route and the linked product/variant titles. Optional **override shipping fee** freezes on `Offer.shippingFeeOverride` and becomes a custom `shippingLine` on the later draft order (blank = Shopify quotes via `draftOrderCalculate` and the cheapest rate is applied with `shippingRateHandle`; `0` = no shipping charge). Omitting `shippingLine` left drafts with no shipping chosen.
...
- Draft order create + invoice send (`createDraftOrderForRequest` in `app/lib/shopify-ops.server.ts`), custom lines priced with `originalUnitPriceWithCurrency`. When the admin left the shipping override blank, the create path quotes via `draftOrderCalculate` (`QuotePlantRequestDraftShipping`) against the customer's default address (`DraftOrderCustomerShipping`) or the shop billing country (`PortalShopShipsFrom`) and applies the cheapest store rate with `shippingRateHandle`. A number, including 0, still uses a custom `shippingLine`.
...
A blank shipping-fee override must not leave `shippingLine` off the draft: Shopify then creates the order with no shipping chosen. The create path quotes available rates through `draftOrderCalculate` and applies the cheapest one by `shippingRateHandle`. An override, including `$0`, still sends a custom `shippingLine` and skips the quote.
...
- Custom draft-order plant lines set `requiresShipping: true` (Shopify defaults a custom line to false, and a draft with nothing shippable quotes no rates). A blank override now quotes via `draftOrderCalculate` and applies `shippingRateHandle`. Confirm the applied rate appears on the invoice during the live draft-order test.
...
3. Customer form: plant name required; notes optional; **no quantity UI**; quantity defaults to 1. **Budget stays out** of the form, customer-facing details, and active workflow. Do not drop `RequestItem.budget` unless a migration is actually required. **Have an existing order?** is required Yes/No; Yes means combine shipping with an existing order and refund overages. The admin **Existing order** filter is New + Yes only.
3a. Send Offer may freeze an optional **shipping fee override** on the offer. Blank quotes via `draftOrderCalculate` and applies the cheapest store rate with `shippingRateHandle` so the later draft uses the shop's usual weight-based shipping. Omitting `shippingLine` left drafts with no shipping chosen. A number, including 0, becomes a custom `shippingLine`. This is carrier shipping, not the FedEx upgrade product.
