...
Demo seed (`ensureShopSeeded`) creates `REQ1`–`REQ7` sample requests plus `REQ8` (declined Thai Constellation + Not Available String of Pearls, Closed) for listing review.
...
- EXACT PLANTS: find/create collection titled `EXACT PLANTS`, `productCreate` with media, vendor **Unsolicited Plant Talks**, and **no Shopify tags**, variant price + weight (lb) + tracked stock of one, `collectionAddProducts`, `publishablePublish` to Online Store and Point of Sale only (paginating all publications). Retries via stored GID also set vendor to **Unsolicited Plant Talks**.
