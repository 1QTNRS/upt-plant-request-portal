...
3a. Send Offer may freeze an optional **shipping fee override** on the offer. Blank leaves `shippingLine` off the later draft order so the customer can choose a store rate at checkout. A number, including 0, becomes a custom `shippingLine` and locks that amount. This is carrier shipping, not the FedEx upgrade product. Do not `draftOrderCalculate` and apply a handle when the field is blank — a preselected rate cannot be changed at checkout.
