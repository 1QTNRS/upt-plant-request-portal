...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    ...
    summary:
      "Two automatic portal emails to the customer — received, then one admin response — plus Shopify's paid-order confirmation; admin types are independently toggleable.",
    detail: [
      ...
      "The customer gets a request-received mail and one admin-response mail when the offer is sent. That response summarises available and unavailable items, includes notes and the hold deadline when plants are available, and links to the offer without claiming payment is due. Accept/Reject still happens in the portal; a Draft Order is created only after they accept. Closing a No Payment Needed request does not send another email.",
      "The third customer email is Shopify's own paid-order confirmation. The portal does not send a payment confirmation, a checkout-link email, or an expiration reminder on the happy path. `checkout_link` is only the admin's manual recovery action.",
