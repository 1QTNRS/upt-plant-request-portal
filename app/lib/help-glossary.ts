...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    detail: [
      ...
      "Link Stock search only returns ACTIVE products published to this shop's Online Store publication. Draft, archived, and POS-or-marketplace-only listings are dropped server-side. Zero-stock ACTIVE Online Store variants stay visible as No stock but cannot be linked. A purchasable variant still needs a price above zero, `availableForSale`, and either untracked stock or at least one unit.",
  ...
  },
  {
    ...
    detail: [
      ...
      "New listings get no Shopify tags from the portal — not `EXACT PLANTS`, not a request number, not `upt-declined-item:{requestItemId}`. The product is added to the existing EXACT PLANTS collection; that membership is the grouping. The Shopify vendor is always **Unsolicited Plant Talks**. Retries find an existing product from the stored Shopify product GID, and still look up older products that already carry `upt-declined-item:{requestItemId}`.",
    ],
    citations: [
      {
        path: "app/lib/exact-plants.ts",
      ...
      {
        path: "app/lib/exact-plants.ts",
        locator: "EXACT_PLANT_VENDOR",
        quote: 'export const EXACT_PLANT_VENDOR = "Unsolicited Plant Talks";',
      },
      {
        path: "app/lib/exact-plants.ts",
