...
describe("iOS admin source layout", () => {
  ...
  it("keeps stock results in an attached dropdown and compact photos", () => {
    ...
    assert.match(editor, /backgroundColor: "#f9ddd9"/);
    assert.match(editor, /borderColor: "#d99a92"/);
  ...
  it("uses native stack swipe-back and a material top tab pager with bottom labels", () => {
    const app = read("App.tsx");
    ...
    assert.match(chrome, /lazy: false/);
